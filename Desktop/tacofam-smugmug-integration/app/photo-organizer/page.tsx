...
import { FolderTree, Upload, Sparkles, Settings, CheckCircle, AlertTriangle, XCircle, Loader, Brain, Archive, RefreshCw, EyeOff, ChevronRight, ChevronDown, Folder, Image as ImageIcon, X, Cloud, FileImage, Info, Zap, Filter, Star, Trash2, Download, BarChart3, Eye, Check, Camera, AlertCircle } from 'lucide-react';
...
export default function PhotoOrganizer() {
  ...
  const [activeTab, setActiveTab] = useState<'build-index' | 'sort-existing' | 'upload-sort' | 'intelligent-culling'>('build-index');
  ...
  const [cullingAlbumKey, setCullingAlbumKey] = useState<string>('');
  const [cullingPhotos, setCullingPhotos] = useState<CullingPhoto[]>([]);
  const [cullingStatistics, setCullingStatistics] = useState<CullingStatistics | null>(null);
  const [isAnalyzingPhotos, setIsAnalyzingPhotos] = useState(false);
  const [cullingProgress, setCullingProgress] = useState<{ current: number; total: number } | null>(null);
  ...
  const [filterMode, setFilterMode] = useState<'all' | 'keeps' | 'rejects' | 'picks' | 'review'>('all');
  ...
  const checkAuthAndInitialize = async () => {
    try {
      ...
      console.error('Photo Organizer: Auth check failed:', error);
      router.push('/');
    }
  };
  ...
  const analyzeUploadedPhotos = async () => {
    ...
    setUploadProgress({ current: 0, total: uploadedFiles.length, fileName: '' });
    setTerminalLogs([]);
    ...
    try {
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        addLog(`   📊 File size: ${(uploadedFile.file.size / 1024 / 1024).toFixed(2)} MB`);
        ...
        try {
          const response = await fetch('/api/ai/analyze-uploaded-photo', {
            ...
            body: JSON.stringify({
              ...
              metadata: {
                ...
                date: uploadedFile.file.lastModified ? new Date(uploadedFile.file.lastModified).toISOString() : null,
              },
              galleryIndex,
            }),
          });
          ...
          if (response.ok) {
            ...
            addLog(`   ✨ AI Vision analysis complete (${analysis.tokensUsed || 'N/A'} tokens used)`);
            addLog(`   👁️ Visual: ${analysis.visualAnalysis?.substring(0, 100)}...`);
            ...
            if (analysis.confidence >= confidenceThreshold) {
              ...
              addLog(`   ✅ AUTO-APPROVED (confidence ≥ ${confidenceThreshold}%)`);
            } else if (analysis.confidence >= 70) {
              status = 'needs-review';
              addLog(`   ⚠️ NEEDS REVIEW (confidence 70-${confidenceThreshold - 1}%)`);
            } else {
  ...
  const loadPhotosForCulling = async (albumKey: string) => {
    if (!albumKey) return;
    ...
    try {
      ...
      const response = await fetch(`/api/smugmug/albums/${albumKey}/images`, {
        headers: {
          'x-access-token': localStorage.getItem('smugmug_access_token') || '',
          'x-access-token-secret': localStorage.getItem('smugmug_token_secret') || '',
        },
      });
      ...
      const photosToAnalyze = images.map((img: any) => ({
      ...
      setCullingProgress({ current: 0, total: images.length });
      ...
      for (let i = 0; i < images.length; i += batchSize) {
        ...
        setCullingProgress({ current: i, total: images.length });

        const analysisResponse = await fetch('/api/ai/analyze-photo-quality', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            ...
            albumContext: galleries.find(g => g.AlbumKey === albumKey)?.Name || '',
            autoRejectThreshold,
      ...
      const analyzedPhotos = photosToAnalyze.map((photo: any) => {
        const analysis = analyses.find((a: any) => a.imageUrl === photo.imageUrl);
        if (analysis) {
          return {
            ...photo,
            ...analysis,
            cullingStatus: analysis.keepRecommendation === 'keep' ? 'keep' :
                          analysis.keepRecommendation === 'reject' ? 'reject' : 'unprocessed',
          };
        }
        return photo;
      });
      ...
      console.error('Error loading photos for culling:', error);
  ...
  const updateCullingStatistics = (photos: CullingPhoto[]) => {
    ...
    const technicalIssuesCount = photos.filter(p =>
    ...
    const totalScore = photos.reduce((sum, p) => sum + (p.qualityScore || 0), 0);
    const avgQualityScore = total > 0 ? (totalScore / total).toFixed(1) : '0';

    const qualityDistribution = {
      excellent: photos.filter(p => p.qualityScore >= 9).length,
      good: photos.filter(p => p.qualityScore >= 7 && p.qualityScore < 9).length,
      average: photos.filter(p => p.qualityScore >= 5 && p.qualityScore < 7).length,
  ...
  const updatePhotoStatus = (index: number, status: 'keep' | 'reject' | 'pick') => {
  ...
  const handleKeyPress = useCallback((e: KeyboardEvent) => {
    ...
    switch(e.key) {
      case 'ArrowRight':
        setSelectedPhotoIndex(prev => Math.min(prev + 1, cullingPhotos.length - 1));
        break;
      case 'ArrowLeft':
      ...
      groups.forEach(group => {
        if (group.length > 1) {
          ...
          group.forEach((photo, index) => {
            const photoIndex = updated.findIndex(p => p.imageKey === photo.imageKey);
            if (photoIndex !== -1) {
              if (index === 0) {
                updated[photoIndex] = { ...updated[photoIndex], cullingStatus: 'keep' };
              } else if (updated[photoIndex].cullingStatus !== 'pick') {
                updated[photoIndex] = { ...updated[photoIndex], cullingStatus: 'reject' };
              }
            }
          });
        }
  ...
  const exportCullingReport = () => {
    const csv = [
      ['Image Name', 'Quality Score', 'Status', 'Keep Recommendation', 'Issues', 'Assessment', 'Key Moment', 'Portfolio Worthy'],
      ...cullingPhotos.map(photo => [
        ...
        [
          ...
          photo.technicalIssues?.exposure !== 'good' && `${photo.technicalIssues?.exposure}`,
  ...
  const applyCulling = async () => {
    ...
    const confirmed = confirm(`This will permanently delete ${rejects.length} photos from SmugMug. Are you sure?`);
    if (!confirmed) return;

    // In production, you would implement the actual deletion logic here
    // For now, we'll show a message
    alert(`Would delete ${rejects.length} photos. This feature requires additional SmugMug API permissions.`);
  };
  ...
  const getFilteredPhotos = () => {
    ...
    if (showSimilarGroups) {
      ...
      filtered.forEach(photo => {
        const hash = photo.similarityHash || `unique-${photo.imageKey}`;
        if (!groups.has(hash) || photo.qualityScore > groups.get(hash)!.qualityScore) {
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-gray-800">
              <span className="font-semibold">How to use:</span> Choose galleries to analyze (AI suggests better organization based on photo content) → Review suggestions with confidence scores and reasoning → Apply approved changes to reorganize your photos with progress tracking.
            </p>
          </div>
        </div>
      </div>
                            ...
                            {expandedFolders.has(folder.NodeID) && (
                              <div className="ml-6 space-y-1">
                                ...
                                {galleries.filter(g => g.ParentNode?.NodeID === folder.NodeID).map((gallery) => {
                                  const isIndexed = galleryIndex.some(idx => idx.albumKey === gallery.AlbumKey);
                                  return (
            ...
            {activeTab === 'upload-sort' && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-600">
                    Upload new photos from your computer and AI will suggest the best gallery for each based on visual content.
                  </p>
                </div>
                ...
                {galleryIndex.length > 0 && (
                  <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                    <div className="flex items-center gap-2 text-sm text-green-800">
                      <CheckCircle className="w-5 h-5 text-green-600" />
                      <span className="font-semibold">Index ready!</span>
                      <span>{galleryIndex.length} galleries available for AI matching</span>
                    </div>
                  </div>
                )}
                      ...
                      {uploadedFiles.map((file) => (
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent p-2 flex flex-col justify-end">
                            <p className="text-xs text-white truncate mb-1">{file.file.name}</p>
                            ...
                            {file.status === 'complete' && (
                              <div className="space-y-1">
                                <div className="flex items-center gap-1 text-xs text-green-300">
                                  <CheckCircle className="w-3 h-3" />
                                  <span className="truncate">{file.suggestedGallery}</span>
                                </div>
                ...
                {uploadProgress && (
                  <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="font-semibold text-indigo-900">
                          Analyzing photo {uploadProgress.current} of {uploadProgress.total}
                        </div>
                        <div className="text-sm text-indigo-700">
                  ...
                  >
                    {isAnalyzingUploads ? (
                      ...
                      <>
                        ...
                        Analyze & Suggest Galleries ({uploadedFiles.length} photos)
                      </>
                    )}
                  </button>
                )}
            ...
            {activeTab === 'intelligent-culling' && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-600">
                    AI-powered photo culling that analyzes quality, composition, and detects issues. Turn hours of culling into minutes.
                  </p>
                </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-200">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-bold text-gray-900 mb-3">Select Album to Cull</h3>
                      <div className="flex gap-3">
                        <select
                          value={cullingAlbumKey}
                          onChange={(e) => setCullingAlbumKey(e.target.value)}
                          className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white"
                        >
                          <option value="">Choose an album...</option>
                          {galleries.map((gallery) => (
                            <option key={gallery.AlbumKey} value={gallery.AlbumKey}>
                              {gallery.Name} ({gallery.ImageCount} photos)
                            </option>
                          ))}
                        </select>
                        <button
                          onClick={() => loadPhotosForCulling(cullingAlbumKey)}
                          disabled={!cullingAlbumKey || isAnalyzingPhotos}
                          className={`px-6 py-3 font-bold rounded-lg transition-all flex items-center gap-2 ${
                            cullingAlbumKey && !isAnalyzingPhotos
                        ...
                        >
                          {isAnalyzingPhotos ? (
                            <>
                              <Loader className="w-5 h-5 animate-spin" />
                              Analyzing... {cullingProgress ? `${cullingProgress.current}/${cullingProgress.total}` : ''}
                            </>
                          ) : (
                            <>
                              <Brain className="w-5 h-5" />
                      ...
                      <div className="mt-4 p-4 bg-white rounded-lg border border-purple-200">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Photos scoring below {autoRejectThreshold}/10 will be flagged for rejection
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-6 text-white">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-sm bg-white/20 px-3 py-1 rounded-full">
                        ⏱ ~{cullingStatistics.estimatedTimeSavedMinutes} minutes saved
                      </span>
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      <div>
                        <div className="text-3xl font-bold">{cullingStatistics.total}</div>
                        <div className="text-sm text-white/80">Total Photos</div>
                      </div>
                      <div>
                        <div className="text-3xl font-bold text-green-300">{cullingStatistics.keeps}</div>
                        <div className="text-sm text-white/80">Keeps ({cullingStatistics.keepRatio}%)</div>
                      </div>
                      <div>
                        <div className="text-3xl font-bold text-red-300">{cullingStatistics.rejects}</div>
                        <div className="text-sm text-white/80">Rejects</div>
                      </div>
                      <div>
                        <div className="text-3xl font-bold text-yellow-300">{cullingStatistics.picks}</div>
                        <div className="text-sm text-white/80">Picks ⭐</div>
                      </div>
                      <div>
                        <div className="text-3xl font-bold">{cullingStatistics.avgQualityScore}</div>
                    ...
                    <div className="mt-4 pt-4 border-t border-white/20">
                      <div className="flex items-center justify-between text-sm">
                        <span>{cullingStatistics.technicalIssuesCount} technical issues</span>
                        <span>{cullingStatistics.keyMoments} key moments</span>
                        <span>{cullingStatistics.portfolioWorthy} portfolio worthy</span>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    ...
                    {viewMode === 'single' && (
                      <div className="mt-3 pt-3 border-t border-gray-200 text-xs text-gray-500">
                        <span className="font-semibold">Keyboard Shortcuts:</span> ← → Navigate • P Pick/Star • X Reject • K Keep • U Undo • G Toggle Groups
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {getFilteredPhotos().map((photo, index) => {
                      const getBorderColor = () => {
                        if (photo.cullingStatus === 'pick') return 'ring-4 ring-yellow-400';
                        if (photo.cullingStatus === 'keep') return 'ring-4 ring-green-500';
                        if (photo.cullingStatus === 'reject') return 'ring-4 ring-red-500 opacity-50';
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="absolute top-2 left-2 right-2 flex flex-wrap gap-1">
                            ...
                            {photo.isBestInGroup && photo.groupSize && photo.groupSize > 1 && (
                ...
                {cullingPhotos.length > 0 && viewMode === 'single' && (
                  <div className="bg-gray-900 rounded-xl p-6">
                    <div className="flex items-start gap-6">
                      ...
                      <div className="flex-1">
                        <img
                          src={getFilteredPhotos()[selectedPhotoIndex]?.imageUrl || getFilteredPhotos()[selectedPhotoIndex]?.thumbnailUrl}
                          alt={getFilteredPhotos()[selectedPhotoIndex]?.imageName}
                      ...
                      <div className="w-96 bg-white rounded-lg p-6 space-y-4">
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-3xl font-bold text-indigo-600">
                              {getFilteredPhotos()[selectedPhotoIndex]?.qualityScore}/10
                            </span>
                        ...
                        {/* Assessment */}
                        <div>
                          <h4 className="font-semibold text-gray-700 mb-1">AI Assessment</h4>
                          <p className="text-sm text-gray-600">
                            {getFilteredPhotos()[selectedPhotoIndex]?.overallAssessment}
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="space-y-1">
                            {getFilteredPhotos()[selectedPhotoIndex]?.technicalIssues?.blur && (
                              <div className="text-sm text-red-600">• Motion blur detected</div>
                            )}
                            {getFilteredPhotos()[selectedPhotoIndex]?.technicalIssues?.exposure !== 'good' && (
                              <div className="text-sm text-red-600">• {getFilteredPhotos()[selectedPhotoIndex]?.technicalIssues?.exposure}</div>
                            )}
                            {getFilteredPhotos()[selectedPhotoIndex]?.technicalIssues?.noise && (
                              <div className="text-sm text-red-600">• Excessive noise/grain</div>
                            )}
                            {getFilteredPhotos()[selectedPhotoIndex]?.subjectIssues?.eyesClosed && (
                              <div className="text-sm text-red-600">• Eyes closed</div>
                            )}
                            {getFilteredPhotos()[selectedPhotoIndex]?.subjectIssues?.awkwardExpression && (
                              <div className="text-sm text-red-600">• Awkward expression</div>
                            )}
                          </div>
                          {!getFilteredPhotos()[selectedPhotoIndex]?.technicalIssues?.blur &&
                           getFilteredPhotos()[selectedPhotoIndex]?.technicalIssues?.exposure === 'good' &&
                           !getFilteredPhotos()[selectedPhotoIndex]?.subjectIssues?.eyesClosed && (
                            <div className="text-sm text-green-600">No major issues detected</div>
                          )}
                        </div>

                        {/* Special Markers */}
                        <div className="flex flex-wrap gap-2">
                          {getFilteredPhotos()[selectedPhotoIndex]?.isPortfolioWorthy && (
                            <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm font-medium">
                              Portfolio Worthy
                            </span>
                          )}
                          {getFilteredPhotos()[selectedPhotoIndex]?.keyMoment && (
                            <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-medium">
                              {getFilteredPhotos()[selectedPhotoIndex]?.keyMoment}
                            </span>
                          )}
                          {getFilteredPhotos()[selectedPhotoIndex]?.isBestInGroup && (
                            <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-medium">
                              Best of {getFilteredPhotos()[selectedPhotoIndex]?.groupSize}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="pt-4 border-t border-gray-200">
                          <div className="grid grid-cols-3 gap-2">
                            <button
                              onClick={() => updatePhotoStatus(selectedPhotoIndex, 'pick')}
                              className={`py-2 rounded-lg font-medium transition-colors ${
                                getFilteredPhotos()[selectedPhotoIndex]?.cullingStatus === 'pick'
                            ...
                            <button
                              onClick={() => updatePhotoStatus(selectedPhotoIndex, 'keep')}
                              className={`py-2 rounded-lg font-medium transition-colors ${
                                getFilteredPhotos()[selectedPhotoIndex]?.cullingStatus === 'keep'
                            ...
                            <button
                              onClick={() => updatePhotoStatus(selectedPhotoIndex, 'reject')}
                              className={`py-2 rounded-lg font-medium transition-colors ${
                                getFilteredPhotos()[selectedPhotoIndex]?.cullingStatus === 'reject'
                        ...
                        <div className="flex items-center justify-between pt-4">
                          ...
                          <span className="text-sm text-gray-500">
                            {selectedPhotoIndex + 1} / {getFilteredPhotos().length}
                          </span>
                          <button
                            onClick={() => setSelectedPhotoIndex(Math.min(getFilteredPhotos().length - 1, selectedPhotoIndex + 1))}
                            disabled={selectedPhotoIndex === getFilteredPhotos().length - 1}
                            className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            Next →
                          </button>
                ...
                {!isAnalyzingPhotos && cullingPhotos.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500">Select an album and click "Load & Analyze" to start culling</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {uploadSortTasks.filter(t => t.status === 'auto-approved').length} auto-approved · {' '}
                  {uploadSortTasks.filter(t => t.status === 'needs-review').length} need review · {' '}
                  {uploadSortTasks.filter(t => t.status === 'skipped').length} skipped
                </p>
              </div>
              <button
              ...
              {uploadSortTasks.map((task, idx) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-gray-900 truncate">{task.fileName}</div>
                      <div className="text-sm mt-2 space-y-2">
                        <div>
                          <span className="text-gray-500 font-medium">Visual Analysis:</span>
                          <p className="text-gray-700 text-xs mt-0.5">{task.visualAnalysis}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-gray-500 font-medium">Suggested Gallery:</span>
                          <span className="font-semibold text-indigo-600">{task.suggestedGallery}</span>
                        </div>
                        <div className="text-xs text-gray-600">{task.reasoning}</div>
                      </div>
                    </div>
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4 flex items-center justify-between">
              ...
              >
                ...
                Upload to SmugMug ({uploadSortTasks.filter(t => t.status === 'auto-approved' || t.status === 'needs-review').length})
              </button>
            </div>
          </div>
        </div>
